...
export const DashboardProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        const res = await fetch('http://192.168.68.79:3000/api/dashboard/getInProgressAllTask', {
          method: 'POST',
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      const year = now.getFullYear();
      try {
        const res = await fetch('http://192.168.68.79:3000/api/dashboard/getFinishedAllTask', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: user.uid, month, year }),
        });
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      const year = now.getFullYear();
      try {
        const res = await fetch('http://192.168.68.79:3000/api/dashboard/getFinishedTaskin8MonthBack', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: user.uid, month, year }),
        });
