...
import KandanBoard from '../../../views/project/components/KandanBoard';
...
export default {
  title: 'Project/Task/KandanBoard',
  component: KandanBoard,
  tags: ['autodocs'],
  argTypes: {
  ...
  decorators: [
    (Story, ctx) => {
      ...
      // Mock useSSE for tasks
      global.useSSE = (url, cb) => {
        if (!url) return;
        setTimeout(() => cb(MOCK_TASKS), 100);
        return null;
      };
...
const Template = (args) => (
  <KandanBoard projectId={null} initialTasks={MOCK_TASKS} />
);
