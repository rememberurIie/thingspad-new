...
const DirectMessageChat = ({ selectedDmId, otherFullName, currentUserId }) => {
  ...
  useSSE(
    selectedDmId ? 'http://192.168.1.32:3000/api/dm/getMessage' : null,
    (data) => {
      switch (data.type) {
        case 'messages': {
  ...
  const handleSend = async () => {
    ...
    try {
      const endpoint = 'http://192.168.1.32:3000/api/dm/sendMessage';
  ...
  const handleDeleteMessage = async (msgId) => {
    try {
      await fetch('http://192.168.1.32:3000/api/dm/deleteMessage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          directMessageId: selectedDmId,
