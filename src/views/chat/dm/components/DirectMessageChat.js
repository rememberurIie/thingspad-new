...
const DirectMessageChat = ({ selectedDmId, otherFullName, currentUserId }) => {
  ...
  useSSE(
    selectedDmId ? 'http://localhost:3000/api/dm/getMessage' : null,
    (data) => {
      switch (data.type) {
        case 'messages': {
  ...
  const handleSend = async () => {
    ...
    try {
      const endpoint = 'http://localhost:3000/api/dm/sendMessage';
  ...
  const handleDeleteMessage = async (msgId) => {
    try {
      await fetch('http://localhost:3000/api/dm/deleteMessage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          directMessageId: selectedDmId,
