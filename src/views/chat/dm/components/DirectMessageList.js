...
const DirectMessageList = ({ onSelect, userId }) => {
  ...
  // --- SSE for DM List ---
  useSSE(
    userId ? 'http://192.168.1.32:3000/api/dm/getDMList' : null,
    (data) => {
  ...
  const handleOpenMenu = async (event) => {
    ...
    try {
      const res = await fetch('http://192.168.1.32:3000/api/dm/getUserToCreateDM', {
        method: 'POST',
  ...
  const handleSelectPerson = async (person) => {
    handleCloseMenu();
    try {
      const res = await fetch('http://192.168.1.32:3000/api/dm/createDM', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId1: userId,
