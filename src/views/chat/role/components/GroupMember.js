...
const ChatMember = ({ onSelect, selectedGroup, currentUserId }) => {
  ...
  useSSE(
    selectedGroup ? 'http://192.168.1.34:3000/api/group/getGroupMember' : null,
    (evt) => {
  ...
  const handleOpenAdd = async () => {
    ...
    setAddError('');
    try {
      const res = await fetch('http://192.168.1.34:3000/api/group/getUserToAdd', {
        method: 'POST',
  ...
  const handleAddUser = async (userId) => {
    ...
    try {
      await fetch('http://192.168.1.34:3000/api/group/toggleUserinGroup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
