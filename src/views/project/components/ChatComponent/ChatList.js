...
const ChatList = ({ onSelect, projectId, selectedRoomId }) => {
  ...
  useSSE(
    projectId ? 'http://192.168.1.32:3000/api/project/chat/getRoomList' : null,
    (evt) => {
  ...
  const handleAddRoom = async () => {
    ...
    try {
      const res = await fetch('http://192.168.1.32:3000/api/project/chat/createRoom', {
        method: 'POST',
  ...
  const handleEditNameSave = async () => {
    ...
    try {
      await fetch('http://192.168.1.32:3000/api/project/chat/updateRoomName', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteRoom = async (roomId) => {
    try {
      await fetch('http://192.168.1.32:3000/api/project/chat/deleteRoom', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
