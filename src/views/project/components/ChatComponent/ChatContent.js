...
const ChatContent = ({
  ...
  useSSE(
    projectId && selectedRoomId
      ? 'http://192.168.68.81:3000/api/project/chat/getMessage'
      : null,
    (data) => {
      switch (data.type) {
        case 'messages': {
  ...
  const handleSend = async () => {
    ...
    try {
      const endpoint = 'http://192.168.68.81:3000/api/project/chat/sendMessage';
  ...
  const handleDeleteMessage = async (msgId) => {
    try {
      await fetch('http://192.168.68.81:3000/api/project/chat/deleteMessage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
