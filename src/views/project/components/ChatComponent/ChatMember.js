...
const ChatMember = ({ onSelect, projectId, currentUserId }) => {
  ...
  // SSE for member list
  useSSE(
    projectId ? 'http://192.168.1.32:3000/api/project/chat/getMemberList' : null,
    (evt) => {
  ...
  const handleOpenAdd = async () => {
    ...
    setAddError('');
    try {
      const res = await fetch('http://192.168.1.32:3000/api/project/chat/getUserNotInProject', {
        method: 'POST',
  ...
  const handleAddUser = async (userId) => {
    ...
    try {
      await fetch('http://192.168.1.32:3000/api/project/chat/toggleUser', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
